...
export class WarehouseVisualizerComponent implements OnInit {
  getAisleGapArray(numAisles: number): number[] {
    if (!numAisles || numAisles <= 1) return [];
  ...
  private createDefaultWorkstation(): any {
    const defaultSideConfig = {
      ...
      aisle_gaps: [50], // Gaps between aisles (num_aisles - 1)
    ...
    return {
      aisle_space: 500,  // Central aisle width (A_W) - matching HTML
      aisle_space_unit: "cm",
  ...
  updateWorkstationConfigs(): void {
    this.warehouseConfig.workstation_configs = this.workstations.map((workstation, index) => {
      ...
      const leftSideConfig: any = {
        ...
        aisle_gaps: convertGaps(workstation.left_side_config.aisle_gaps || [], workstation.left_side_config.wall_gap_unit || "cm"),
        deep_gaps: convertGaps(workstation.left_side_config.deep_gaps || [], workstation.left_side_config.wall_gap_unit || "cm"),
        wall_gap_unit: "cm",
      };
      ...
      const rightSideConfig: any = {
        ...
        aisle_gaps: convertGaps(workstation.right_side_config.aisle_gaps || [], workstation.right_side_config.wall_gap_unit || "cm"),
        deep_gaps: convertGaps(workstation.right_side_config.deep_gaps || [], workstation.right_side_config.wall_gap_unit || "cm"),
        wall_gap_unit: "cm",
      };
  ...
  updateAisleGaps(workstationIndex: number, side: 'left' | 'right'): void {
    ...
    // Initialize aisle gaps (num_aisles - 1)
    const requiredAisleGaps = Math.max(0, numAisles - 1);
    const currentAisleGaps = sideConfig.aisle_gaps || [];
    const newAisleGaps = Array(requiredAisleGaps).fill(50); // Default 50cm gap
    
    // Preserve existing aisle gap values
    for (let i = 0; i < Math.min(currentAisleGaps.length, newAisleGaps.length); i++) {
      newAisleGaps[i] = currentAisleGaps[i];
    }
    
    // Initialize deep gaps (depth - 1)
    const requiredDeepGaps = Math.max(0, depth - 1);
    const currentDeepGaps = sideConfig.deep_gaps || [];
    const newDeepGaps = Array(requiredDeepGaps).fill(50); // Default 50cm gap
    
    // Preserve existing deep gap values
    for (let i = 0; i < Math.min(currentDeepGaps.length, newDeepGaps.length); i++) {
      newDeepGaps[i] = currentDeepGaps[i];
    }

    sideConfig.aisle_gaps = newAisleGaps;
    sideConfig.deep_gaps = newDeepGaps;
    this.updateWorkstationConfigs();
  }
    ...
    this.warehouseService.createWarehouse(this.warehouseConfig).subscribe({
      ...
      error: (error) => {
        ...
        if (typeof error === 'object') {
          errorMsg = error.message || error.error?.detail || String(error);
        } else {
          errorMsg = String(error);
        }
        console.error("Error creating warehouse:", errorMsg);
        this.setStatus(
          `Error: ${errorMsg}`,
          "text-danger"
        );
